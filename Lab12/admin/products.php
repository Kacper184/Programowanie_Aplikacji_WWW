...
<body>
    ...
    {
        echo "<form method='post' action='products.php' enctype='multipart/form-data'>";
        ...
        echo "<input type='file' name='zdjecie' accept='image/jpeg, image/png'><br><br>";
    ...
    function UsunProdukt()
    {
        ...
        echo "<br><label>Id Produktu: </label><br>";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['Add_kat'])) {
        ...
        $sql = "INSERT INTO `produkty` (`id`, `tytul`, `opis`, `data_utworzenia`, `data_modyfikacji`, `data_wygasniecia`, `cena_netto`, `podatek_vat`, `ilosc_sztuk`, `status_dostepnosci`, `kategoria_produktu`, `gabaryt_produktu`, `zdjecie_produktu`) VALUES (NULL, '$name', '$opis', '$data_ut', '$data_mod', '$data_wyga', '$netto', '$vat', '$sztuki', '$status', '$kategoria', '$gabaryt', '$zdjecie') LIMIT 1";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['del_kat'])) {
        ...
        $sql = "DELETE FROM `produkty` WHERE `produkty`.`id` = $id LIMIT 1";
        if ($link->query($sql) === TRUE) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_kat'])) {
        ...
        $sql = "UPDATE `produkty` SET `tytul` = '$name', `opis` = '$opis', `data_modyfikacji` = '$data_mod', `cena_netto` = '$netto', `podatek_vat` = '$vat', `ilosc_sztuk` = '$sztuki', `status_dostepnosci` = '$status', `kategoria_produktu` = '$kategoria ', `gabaryt_produktu` = '$gabaryt', `zdjecie_produktu` = '$zdjecie' WHERE `produkty`.`id` = $id LIMIT 1";
        if ($link->query($sql) === TRUE) {
            echo "Dodano zawartość do bazy danych.";
            header('Location: products.php');
        } else {
